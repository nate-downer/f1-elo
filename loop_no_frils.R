...
#' this is the no frills loop so that I have a bseline to compare against
#' it has no decay, matchmaking, or scaled k values but the
#' teams and drivers elos are clculated at the same time
...
run_elo_no_frils <- function(start_year, k_value, match_rage) {
  ...
  for (this_race in races_limit$raceOrder) {
    ...
    elo_results_limit <- elo_results_limit %>%
      merge(
        get_race_results(this_race_results, current_driver_elos, current_team_elos, k_value, match_range = match_rage) %>%
...
# elo_results_no_frils <- run_elo_no_frils(1950, 32, 5000)
# elo_results_no_frils_1 <- run_elo_no_frils(1950, 32, 500)

elo_results_no_frils_2 <- run_elo_no_frils(1950, 32, 200)



## clean the results ----

long_elo_results_limit <- elo_results_no_frils_2 %>%
...
tagged_elo_results_limit %>%
  ...
  hc_title(text = "Top Team Elo Over Time (No Frills)", style = list(fontSize = "14px"))
...
tagged_elo_results_limit %>%
  ...
  hc_legend(enabled = FALSE) %>%
  hc_title(text = "Top Driver Elo Over Time (No Frills)", style = list(fontSize = "14px"))
...
tagged_elo_results_limit %>%
  ...
  hc_title(text = "Average Elo Over Time")

bottom_driver_list <- tagged_elo_results_limit %>%
  filter(!is.na(driver)) %>%
  group_by(contRaceOrder) %>%
  arrange(display_elo) %>%
  slice_min(display_elo, n = 1) %>%
  ungroup() %>%
  distinct(driver) %>%
  mutate(row_num = as.numeric(row_number()))

tagged_elo_results_limit %>%
  filter(driver %in% bottom_driver_list$driver) %>%
  hchart(type = "line",
          hcaes(
            x = as.numeric(contRaceOrder),
            y = display_elo,
          ...
          tooltip = list(pointFormat = "<b>{point.driver}</b> as of: {point.race}")) %>%
  hc_legend(enabled = FALSE) %>%
  hc_title(text = "Bototm Driver Elo Over Time (No Frills)", style = list(fontSize = "14px"))
